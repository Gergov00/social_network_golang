...
func (r *RefreshRepo) Create(ctx context.Context, token *domain.RefreshToken) error {
	...
	_, err := r.pool.Exec(ctx, query, token.ID, token.UserID, token.TokenHash, token.FamilyID, token.UsedAt, token.RevokedAt, token.ExpiresAt, token.UserAgent, token.IP, token.CreatedAt)
	if err != nil {
		return err
	}
	return nil
...
func (r *RefreshRepo) MarkUsed(ctx context.Context, token *domain.RefreshToken) error {
